    ...
    def get_OHLCV(self,time_tuple,tf='1h'):
        ...
        if(len(OHLCV)):
            ...
            for symbol in symbols:
                    ...
                    since =exchange.parse8601(OHLCV[OHLCV['symbol']==symbol].Date.max())
            ...
            mycol = db[ooo]
            print(ooo)
            data = b.to_dict(orient='records')  # Here's our added param..
            mycol.insert_many(data)
            OHLCV=pd.concat([a,OHLCV],ignore_index=True)
            OHLCV=OHLCV.drop_duplicates()
            print('OHLCV loaded from DB')
            a=0
                   # time.sleep (exchange.rateLimit / 2000)
            ...
            for symbol in symbols:
                    ...
                    a['change']=self.comp_prev(a)
                    OHLCV=pd.concat([a,OHLCV],ignore_index=True)
                    since = exchange.milliseconds () - (120*86400000)
